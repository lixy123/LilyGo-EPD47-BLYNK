...
class XFS5152CE
{
  public:
    ...
    */
    ...
    bool IIC_WriteByte(int data);
    
    //void IIC_WriteBytes(uint8_t* buff, uint32_t size);
    //void IIC_WriteBytes(const char * buff, uint32_t size);
    void IIC_WriteBytes(const char * buff, int size);

    void Begin_with_pin(uint8_t addr, int sda_pin, int scl_pin);
    ...
    XFS5152CE(EncodingFormat_Type encodingFormat = UNICODE);
  ...
  private :
    ...
    uint16_t utf16_buff[100];  //100个汉字的量(utf16格式保存)

    //必须有所有函数引用
    String tohex_string(const uint8_t * cmd_buff, int sourceLen);
    String tohex(uint8_t n) ;
    void utf8ToUtf16String(String gUtf8ZhString,  size_t &size_char );
    void utf32ToUtf16String( uint32_t *  utf32Str, uint16_t * utf16Str, size_t  strSize);
